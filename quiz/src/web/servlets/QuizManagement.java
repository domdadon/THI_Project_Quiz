...
public class QuizManagement extends HttpServlet {
	...
	private String loginpage = "./html/login.jsp";
		...
		try {
			if (action == null) {
				...
				case "register":
					...
					break;
				case "registerpage":
					dispatch(request, response,"registerpage", userID);
					break;
				case "loginpage":
					dispatch(request, response,"loginpage", userID);
					break;
				...
				case "getNextQuestion":
					qb = getNextQuestion(gameID, categoryID);
					// Nach zehn Fragen keine neue Bean -> zur�ck zur landing
					if (qb == null) {
						dispatch(request, response,"landing", userID);
					}
					else 
	...
	protected QuestionBean getNextQuestion(Integer gameID, Integer categoryID) throws Exception {
		...
		try (Connection cnx = ds.getConnection()) {
			...
			//Aktueller Fragenz�hler im Quiz anzeigen
			...
			try {
				...
				if (rs != null) {
					//Bef�llung der QuestionBean mit Antworten
				...
				//"Abfangen" des Zur�ckbuttons im Browser
	...
	private Boolean checkAnswer(Integer questionID, Integer answerID, Integer gameID) throws Exception {
		try (Connection cnx = ds.getConnection()) {
			...
			Boolean result = false;
			//Pr�fen ob die abgegebene Antwort richtig ist oder nicht 
			...
			rs = sql.executeQuery();
			//Bei richtiger Score hochz�hlen mittels categoryID
			if (rs != null && rs.next()) {
	...
	private UserBean getUserData(Integer UserID, Boolean complete) throws Exception {
		...
		try (Connection cnx = ds.getConnection();) {
			...
			//Alle Userdaten nur bei der Statistikseite laden -> Performancegr�nde
			if (complete) {
				...
				//Anzahl der richtigen und falschen Antworten f�r Quote
